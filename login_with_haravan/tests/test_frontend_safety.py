...
class TestFrontendSafety(unittest.TestCase):
    ...
    def test_haravan_org_selector_autofills_and_injects_contact_phone(self):
        ...
        self.assertIn("setSelectedOrg(orgs[0])", content)
        self.assertIn("input[name='custom_contact_phone']", content)
        self.assertIn("docObj.custom_phone = phone", content)
        self.assertIn("docObj.custom_contact_phone = phone", content)
        self.assertIn("jsonBody.doc.custom_phone = phone", content)
        self.assertIn("jsonBody.doc.custom_contact_phone = phone", content)
    ...
    def test_haravan_org_selector_fetches_without_frappe_call(self):
        ...
        self.assertIn("fetchOrgs(injectSelector);", content)
